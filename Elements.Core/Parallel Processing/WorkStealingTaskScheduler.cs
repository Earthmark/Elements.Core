...
// Adapted from: https://github.com/n-ski/ParallelExtensionsExtras.NetFxStandard/blob/master/src/ParallelExtensionsExtras.NetFxStandard/TaskSchedulers/WorkStealingTaskScheduler.cs
...
public class WorkStealingTaskScheduler : TaskScheduler, IDisposable
{
    ...
    public WorkStealingTaskScheduler(ThreadPriority threadPriority = ThreadPriority.Normal) : this(Environment.ProcessorCount * 2, threadPriority) { }

    /// <summary>Initializes a new instance of the <see cref="WorkStealingTaskScheduler"/> class.</summary>
    /// <param name="concurrencyLevel">The number of threads to use in the scheduler.</param>
    public WorkStealingTaskScheduler(int concurrencyLevel, ThreadPriority threadPriority = ThreadPriority.Normal, string threadName = null)
    {
    ...
    protected override void QueueTask(Task task)
    {
        ...
        {
            new Thread(state => base.TryExecuteTask((Task?)state!)) { IsBackground = true, Priority = m_threadPriority, Name = m_threadName + "LongRunning" }.Start(task);
        }
        else
        {
